...
class AI:
    ...
    def control(self, sqs_given, player_status):
        if sqs_given.curr_sq == sqs_given.st.new or self.direction is None:
            self.direction = make_choice(sqs_given)
        else:
            move(sqs_given, self.direction, player_status)


def move(sqs_given, direction, player_status):
    # rotation
...
def get_end_pos_with_rotate(pos, sqs):
    ...
    while old_sq != sqs.curr_sq and sqs.curr_sq[1] < square_num_x - 1:
...
def move_sq_to_left(sqs):
    old_sq = None
    while old_sq != sqs.curr_sq and sqs.curr_sq[1] > 0:
        old_sq = sqs.curr_sq
        sqs.left(sqs)
...
def map_pos_to_sqs(sqs, positions):
    for pos in positions:
        sqs.squares[pos[0]][pos[1] % square_num_x] = 'map'
