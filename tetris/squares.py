...
class Squares:
    ...
    def __init__(self, st, status, player_status, screen, empty_line=None, squares=None, player=None):
        self.player = player
    ...
    def update(self):
        ...
        if self.player_status.straight_drop and self.clock.is_time_to_straight_drop():
            updated = True
        ...
        elif self.player_status.down and self.clock.is_time_to_quick_drop():
            updated = True
            self.drop(self)
            self.clock.update_quick_drop()
        # rotation
        if self.player_status.rotate and self.clock.is_time_to_rotate():
            updated = True
            self.rotate(self)
        ...
        if self.player_status.right:
            updated = True
            if self.clock.is_time_to_quick_right():
        ...
        if self.player_status.left:
            updated = True
            if self.clock.is_time_to_quick_left():
                if self.clock.is_time_to_move_quick():
                    self.left(self)
                    self.clock.update_move_quick()
            else:
    ...
    @staticmethod
    def new_sq(self, init=False):
        self.curr_sq = [1, randrange(self.st.square_num_x)]
    ...
    def rotate(self):
        ...
        # move horizontally if not valid
            ...
            for i in range(tolerance):
                ...
                new_sq_left[1] = self.decrement_x(self, new_sq_left[1])
                ...
                new_sq_right[1] = self.increment_x(self, new_sq_right[1])
    ...
    def right(self):
        ...
        new_sq[1] += 1
        if self.valid(self, new_sq, self.curr_shape):
            self.curr_sq = self.wrap(self, new_sq)
    ...
    def wrap_coords(self, x, y):
        return x % self.st.square_num_x, y
    ...
    def stop(self):
        ...
        for sq in self.curr_shape:
            ...
            if y >= 0:
                self.squares[y][self.wrap_x(self, x)] = self.curr_color
        x = self.curr_sq[1]
        y = self.curr_sq[0]
        if y >= 0:
            self.squares[y][self.wrap_x(self, x)] = self.curr_color
    ...
    def valid_sq(self, sq):
        ...
        if sq[0] >= self.st.square_num_y:  # or sq[1] >= self.st.square_num_x or sq[1] < 0:
            return False
        # check crash
        return self.squares[sq[0]][self.wrap_x(self, sq[1])] == 'none'
